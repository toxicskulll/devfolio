...
import styles from "./ResearchTile.module.scss";

const ResearchTile = ({ research, classes, isDesktop }) => {
  const { name, image, blurImage, description, gradient, url, tech } = research;
  const researchCard = useRef(null);
  ...
  useEffect(() => {
    VanillaTilt.init(researchCard.current, options);
  }, [researchCard]);
  ...
  return (
    ...
    >
      <div
        className={`h-[25rem] w-[38rem] bg-black ${styles.ResearchTile} rounded-3xl relative p-6 flex flex-col justify-between max-w-full`}
        style={{
      ...
      >
        ...
        <Image
          ...
          className={`${styles.researchImage} z-0`}
        />
        <div
